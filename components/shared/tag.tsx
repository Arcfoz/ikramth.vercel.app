...
const tagVariants = cva("inline-flex h-6 max-w-max cursor-default items-center justify-start gap-1 rounded-[64px] px-2 py-1 text-center text-xs font-semibold leading-relaxed", {
  variants: {
    variant: {
...
export function Tag({ className, startIcon, endIcon, variant, children, ...rest }: TagProps) {
  return (
    <div className={clsxMerge(tagVariants({ variant, startIcon: Boolean(startIcon), endIcon: Boolean(endIcon) }), className)}>
      {startIcon && <div className="inline-flex size-[18px] items-center justify-start overflow-hidden">{startIcon}</div>}
